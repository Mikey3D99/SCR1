...
int main() {
    // Initialize logging with a log file name, log level, and state dump callback
    init_logging("app.log", STANDARD, state_dump);

    // Log messages with different importance levels
    log_message(MIN, "This is a MIN level log message.");
    log_message(STANDARD, "This is a STANDARD level log message.");
    log_message(MAX, "This is a MAX level log message.");

    // Simulate the application running
    printf("Application running...\n");
    sleep(5);

    // Close logging before exiting the application
    close_logging();
